...
const JaisalmerPage = () => {
  ...
  useEffect(() => {
    fetch("/data/jaisalmerData.json")
      .then((res) => res.json())
      .then((json) => setData(json))
      .catch((err) => console.error("Failed to load Jaisalmer data:", err));
  }, []);

  if (!data)
    return <p style={{ textAlign: "center", padding: "50px" }}>Loading Jaisalmer information...</p>;

  const handleBooking = (type, name) => {
    navigate("/booking", { state: { city: "Jaisalmer", [type]: name } });
  };

  return (
    <div className="city-page">
      <h1 className="city-heading">Explore Jaisalmer</h1>
