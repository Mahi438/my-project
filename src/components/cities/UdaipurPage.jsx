...
const UdaipurPage = () => {
  ...
  useEffect(() => {
    fetch("/data/udaipurData.json")
      .then((res) => res.json())
      .then((json) => setData(json))
      .catch((err) => console.error("Failed to load Udaipur data:", err));
  }, []);

  if (!data)
    return <p style={{ textAlign: "center", padding: "50px" }}>Loading Udaipur information...</p>;

  const handleBooking = (type, name) => {
    navigate("/booking", { state: { city: "Udaipur", [type]: name } });
  };

  return (
    <div className="city-page">
      <h1 className="city-heading">Explore Udaipur</h1>
